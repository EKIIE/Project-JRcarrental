...
$q = mysqli_query($conn, "SELECT inst_date, monthly, company, url, receipt, note
                          FROM installments WHERE car_id={$car_id}
                          ORDER BY inst_date DESC, installment_id DESC");
...
echo '<div class="table-responsive"><table class="table table-sm table-striped align-middle">
        <thead><tr>
          <th>วันที่ชำระ</th><th>บริษัทไฟแนนซ์</th><th class="text-end">ยอด/เดือน</th><th>หลักฐาน</th><th>หมายเหตุ</th>
        </tr></thead><tbody>';
while ($r = mysqli_fetch_assoc($q)) {
  $link = $r['url'] ? '<a href="'.htmlspecialchars($r['url']).'" target="_blank">link</a>' : '';
  $rcpt = $r['receipt'] ? '<a href="../uploads/installments/'.htmlspecialchars($r['receipt']).'" target="_blank">ไฟล์</a>' : '';
  echo '<tr>';
  echo '<td>'.htmlspecialchars(date('d/m/Y', strtotime($r['inst_date']))).'</td>';
